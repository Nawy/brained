...
pub const DEFAULT_CONFIG_TOML: &str = "chunk_size = 800\nscan_interval_seconds = 30\nignore = []\n";
...
pub fn load_config(config_path: &Path) -> anyhow::Result<Config> {
    ...
    let raw: RawConfig = toml::from_str(&text)
        .with_context(|| format!("parsing {} (check the `ignore` key is a list)", config_path.display()))?;
    Ok(Config {
...
ignore = ["*.tmp"]"#
    ...
    /// Hard cutover: `knowledge_folders` used to be a config key (pre business/tech
    /// restructuring). Old configs that still have it must keep parsing — serde silently
    /// ignores unknown fields since `RawConfig` has no `deny_unknown_fields` — the key just
    /// no longer has any effect (the four folders are hardcoded now).
    #[test]
    fn stray_knowledge_folders_key_is_silently_ignored() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join(".brained.config.toml");
        std::fs::write(&path, "knowledge_folders = [\"old-docs\"]\nchunk_size = 123\n").unwrap();
        let config = load_config(&path).unwrap();
