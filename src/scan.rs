...
pub async fn scan_once(state: &mut SharedState) -> anyhow::Result<()> {
    let discovered = walk_folders(&state.root, &state.config, &state.spec)?;
...
async fn scan_one_file(state: &mut SharedState, file: &crate::walk::DiscoveredFile) -> anyhow::Result<bool> {
    ...
    state.vector_store.delete_chunks_for_path(&file.rel_path).await?;
    if !chunks.is_empty() {
        state.vector_store.add_chunks(&file.rel_path, file.file_type, &chunks, &vectors).await?;
    }
...
mod tests {
    ...
    async fn scan_indexes_new_files_and_removes_stale_ones() {
        ...
        let results = state.vector_store.query_chunks(&[0.0; 4], None, 10).await.unwrap();
    ...
    async fn unchanged_file_is_not_rescanned() {
        ...
        scan_once(&mut state).await.unwrap();
        let before = crate::store::get_file_record(&*state.conn.lock().await, "a.md").unwrap().unwrap();
        scan_once(&mut state).await.unwrap();
        let after = crate::store::get_file_record(&*state.conn.lock().await, "a.md").unwrap().unwrap();
    ...
    async fn unreadable_file_is_skipped_not_fatal() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(dir.path().join("good.md"), "hello world").unwrap();
        std::fs::write(dir.path().join("bad.md"), [0xFF, 0xFE, 0xFD]).unwrap(); // invalid UTF-8 — read_to_string will genuinely fail
        let mut state = build_state(dir.path(), Config::default()).await;
    ...
    async fn audio_files_are_converted_and_indexed() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(dir.path().join("memo.mp3"), "fake mp3 bytes").unwrap();
        ...
        let results = state.vector_store.query_chunks(&[0.0; 4], None, 10).await.unwrap();
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].path, "memo.mp3");
